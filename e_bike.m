classdef e_bike  %自行车类
    %e-bikes are the object which run on roads
    ...
    properties      %变量名
        ...
        Va_y = 5.2;     %期望速度 
        Ta = 1;       %个体反应时间
        Aa1 = 500;     %自行车对个体的作用力强度系数
        Aa2 = 700;     %行人对个体的作用力强度系数
        Ba1 = 4.0;       %个体的心理力作用范围
        Ba2 = 6.2;
        r;      %个体半径
        Ao = 200;     %障碍物排斥力强度系数
  ...
  %  end
    ...
    methods
        function AM = e_bike(t0,N,Profile,Fa,Fb,Fc,F,U,A,q,Ta,Aa1,Aa2,Ba,Bo,r,Ao,m,V,a,b,I,ko,kd,kl,alpha)%Construction functionr构造函数
            if nargin > 0
                AM.t0 = t0;
                AM.Name = e_bike;
                AM.N = N;
        ...
        function [U,F,A,Fa,Fb,Fc] = speed3(obj,argu1,argu2,t,e1,e2)
            t_r = int16(10*t);
            t_r_old = int16(t_r - 1);
            ...
            for j = 1:length(argu2)
                if argu2{1,j}.Profile(1,t_r)>e1 && argu2{1,j}.Profile(1,t_r)<e2 %&& obj.Profile(1,t_r)<argu2{1,j}.Profile(1,t_r)
        ...
        %更新位置
        function Profile = position3(oldspeed_x,oldspeed_y,obj,t)
            t_r = int16(10*t);
            Profile = zeros(2,1);
