...
for  i = 1 : length(P_collection)
    ...
    P_collection{1,i}.destination_x = 100;
    ...
    P_collection{1,i}.attention_log(1,1) = 20;
...
for n  = 0.2 : step_sim : t        %  time step 1/r second
    ...
    if whether_RL==1;
        ...
        for j = 1:length(P_collection)
            ...
            reward_t = - ( w_Ec  * Ec_now   ...
                         - w_v   * speedErr ...
                         - w_rho * rho_now  );   % 越小越罚，越大越奖
    ...
    %% ---------- 统计已完成行人的旅行时间 ----------
    travelTime = nan(1,ped);          % 预分配
    exitX      = 59;                  % 通道右端 x 坐标
    
    for j = 1:ped
        % 找到该行人第一次越过出口的时刻
        idxExit = find(P_collection{1,j}.Profile(1,:) >= exitX, 1, 'first');
        
        if ~isempty(idxExit)                         % 成功到达
            tExit = (idxExit-1)/r;                   % step → 连续时间
            travelTime(j) = tExit - P_collection{1,j}.t0;
        end
end

% 去除 NaN，只统计已完成者
finishedMask   = ~isnan(travelTime);
meanTravelTime = mean(travelTime(finishedMask));

fprintf('\n>>> 已完成行人数: %d / %d\n', sum(finishedMask), ped);
fprintf('>>> 平均旅行时间: %.2f s\n\n', meanTravelTime);
end
...
total_energy_loss=sum(energyLoss);
